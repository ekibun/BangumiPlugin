...
class LineDialog(private val linePresenter: LinePresenter) :
    ...
    private fun updateProvider(view: View) {
        view.item_line.setOnClickListener {
            ...
            val providerList: ArrayList<LineProvider.ProviderInfo> =
                ArrayList(linePresenter.lineProvider.providerList.values.filter { it.type == linePresenter.type })
                ...
                when (position) {
                    providerList.size - 3 -> {
                        ...
                        linePresenter.loadProvider(linePresenter.type, null) {
                            ...
                            linePresenter.lineProvider.addProvider(it)
                    ...
                    providerList.size - 2 -> {
                        ...
                        clipboardManager.setPrimaryClip(
                            ClipData.newPlainText(
                                ...
                                JsonUtil.toJson(linePresenter.lineProvider.providerList.values)
                            )
                        )
                    ...
                    providerList.size - 1 -> {
                        val addProvider = { it: LineProvider.ProviderInfo ->
                            val oldProvider = linePresenter.lineProvider.getProvider(linePresenter.type, it.site)
                            if (oldProvider != null)
                            ...
                            else linePresenter.lineProvider.addProvider(it)
                        }
                        //inport
            ...
            popList.listView?.setOnItemLongClickListener { _, _, position, _ ->
                ...
                linePresenter.loadProvider(linePresenter.type, info) {
                    linePresenter.lineProvider.removeProvider(info)
                    if (it != null) linePresenter.lineProvider.addProvider(it)
                    updateProvider(view)
                }
                true
            }
    ...
    private fun updateInfo(view: View, info: LineInfoModel.LineInfo) {
        val provider = linePresenter.lineProvider.getProvider(linePresenter.type, info.site) ?: emptyProvider
        view.item_line.text = provider.title
        view.item_line.tag = provider
