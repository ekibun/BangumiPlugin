...
class MangaAdapter(data: MutableList<MangaProvider.ImageInfo>? = null) :
    ...
    private fun loadData(helper: BaseViewHolder, item: MangaProvider.ImageInfo) {
        ...
        if (imageRequest != null) {
            ...
            (App.app.lineProvider.getProvider(Provider.TYPE_MANGA, item.site ?: "")?.provider as? MangaProvider)
                ?.getImage("${item.site}_${item.id}", App.app.jsEngine, item)?.enqueue({
                    requests[item] = it
