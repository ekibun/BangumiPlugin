...
class SearchDialog(private val linePresenter: LinePresenter) :
    ...
    override fun onViewCreated(view: View) {
        ...
        val adapter = SearchLineAdapter(linePresenter.type)
        adapter.lines = linePresenter.lineInfoModel.getInfos(subject)
        ...
        view.item_search.setOnClickListener {
            ...
            searchCall.addAll(if (provider == emptyProvider) {
                ArrayList(linePresenter.lineProvider.providerList.values.filter { it.type == linePresenter.type &&  !it.provider?.search.isNullOrEmpty() })
                    .map {
