...
var window={
    eval:(result)=>{
        throw JSON.parse(result.replace("SMH.reader(", "").replace(").preInit();", ""))
    }
}
var NativeAd=()=>{}
try{
    eval(doc.select("script").html())
}catch(e){
    info = e
}
var token = "?cid="+info.chapterId+"&md5="+info.sl.md5
return info.images.map(path => ({
    site: "manhuagui",
    image: {
        url: encodeURI("https://i.hamreus.com" + path + token),
        header: {
